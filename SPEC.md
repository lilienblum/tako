...
**Instance identity (CLI args, not env vars):** `tako-server` passes per-instance identity to the SDK entrypoint as command-line arguments, not environment variables:

- `--instance <id>` — 8-character nanoid instance identifier
- `--version <version>` — build/version identifier (matches `TAKO_BUILD`)

The SDK parses these from `process.argv` (JS) or `os.Args` (Go) at startup and exposes them through the internal status endpoint and health-check responses.
