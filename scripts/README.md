...
- `setup-tako-cli.sh`: POSIX installer for local `tako` CLI.
- `setup-tako-server.sh`: POSIX installer for `tako-server` on Linux hosts.
...
```bash
sh scripts/setup-tako-cli.sh
sh scripts/setup-tako-server.sh
bash scripts/check_critical_coverage.sh
```

The install scripts are served by website endpoints:
