...
- `tako deploy` packages source files from the app's source root (git root when available; otherwise nearest workspace root or app directory), filtered by `.gitignore`.
- `tako deploy` always excludes `.git/`, `.tako/`, `.env*`, `node_modules/`, and `target/` from source bundles.
- `tako deploy` resolves a build preset (`[build].preset`, default `bun`) and locks resolved preset source metadata in `.tako/build.lock.json`.
